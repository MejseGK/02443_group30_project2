...
for p = 1:length(N_sizes)
    ...
    % GRAPH FORMATTING
    title(['Influenza Simulation: N = ', num2str(N)], 'FontSize', 16); % Extra large for title
    xlabel('Time (days)', 'FontSize', 14);
    ylabel('Proportion of Population', 'FontSize', 14);
    
    % This line increases the size of the axis tick numbers and matches the labels
    set(gca, 'FontSize', 14); 
...
for p = 1:length(N_sizes)
    ...
    % GRAPH FORMATTING
    title(['COVID-19 Simulation: N = ', num2str(N)], 'FontSize', 16); % Extra large for title
    xlabel('Time (days)', 'FontSize', 14);
    ylabel('Proportion of Population', 'FontSize', 14);
...
R0 = 1.5;           % Basic reproduction number
gamma = 0.10;       % Recovery rate (days^-1) -> 10 days infectious window
beta = 0.15;        % Infection rate (days^-1)
...
for p = 1:length(N_sizes)
    ...
    h_ode_S = plot(time_steps, S_det, 'b--', 'LineWidth', 2.0);
     ...
     % GRAPH FORMATTING
    title(['Ebola Simulation: N = ', num2str(N)], 'FontSize', 16); % Extra large for title
    xlabel('Time (days)', 'FontSize', 14);
    ylabel('Proportion of Population', 'FontSize', 14);
